...
from .networks import Discriminator, HazeRemovalNet,HazeProduceNet, HazeProduceNet_real,  DepthEstimationNet
from .loss import ContrastLoss, AdversarialLoss
...
class Model(BaseModel):
    def __init__(self, config):
        ...
        if self.config.IS_REAL_MODEL == 0:
            ...
            self.net_c2h = HazeProduceNet_real(config.BASE_CHANNEL_NUM // 2, in_channels=3, out_channels=3,
                                          min_beta=config.MIN_BETA, max_beta=config.MAX_BETA, min_d=config.MIN_D, max_d=config.MAX_D)
    ...
    def forward_depth_for_visual(self, clean_imgs):
    ...
    def forward_c2h_given_parameters_real(self, clean_imgs, ex, beta, A=None, rearrange=False):
        x = self.net_c2h.forward(clean_imgs,ex,beta, A, rearrange)
        return x

    def forward_c2h_random_parameters_real(self, clean_imgs, ex, requires_paras=False, rearrange=False): # random beta
        x, beta = self.net_c2h.forward_random_parameters(clean_imgs, ex, rearrange=rearrange)
        if requires_paras:
            return x, beta
        else:
            return x
    ...
    def process_outdoor(self, clean_images, hazy_images, clean_2, hazy_2, mask_clean, mask_hazy):
        self.iteration += 1
        ...
        A_h = self.net_h2c.dcgf_tool.get_atmosphere_light_new(hazy_images)
        clean_images_h2c, gt_ed_h2c, pred_beta_h2c = self.forward_h2c(hazy_images, require_paras=True)
        pred_ed_h2c = self.forward_depth(clean_images_h2c)
        if self.iteration>500:
            hazy_images_h2c2h = self.forward_c2h_given_parameters_real(clean_images_h2c, pred_ed_h2c, pred_beta_h2c,A=A_h,rearrange=True)
        else:
            hazy_images_h2c2h = self.forward_c2h_given_parameters_real(clean_images_h2c, pred_ed_h2c, pred_beta_h2c,A=A_h,rearrange=False)


        pred_ed_clean = self.forward_depth(clean_images)
        if self.iteration > 500:
            hazy_images_c2h, beta_gt_c2h = self.forward_c2h_random_parameters_real(clean_images, pred_ed_clean, requires_paras=True, rearrange=True)
        else:
            hazy_images_c2h, beta_gt_c2h = self.forward_c2h_random_parameters_real(clean_images, pred_ed_clean, requires_paras=True, rearrange=False)
        ...
        dis_real_clean, _ = self.discriminator_h2c(
            (1-mask_clean.detach())*clean_images)
        dis_fake_clean, _ = self.discriminator_h2c(
            (1 - mask_hazy.detach()) * clean_images_h2c.detach())
        ...
        cycle_loss_c2h2c = self.l1_loss((1-1*mask_clean.detach())*clean_images,
                                        (1-1*mask_clean.detach())*clean_images_c2h2c)
        cycle_loss_h2c2h = self.l1_loss(hazy_images, hazy_images_h2c2h)
        cycle_loss = (cycle_loss_c2h2c + cycle_loss_h2c2h)
        ...
        gen_fake_haze, _ = self.discriminator_c2h(
            hazy_images_c2h)
        gen_fake_clean, _ = self.discriminator_h2c(
            (1-mask_hazy.detach())*clean_images_h2c)
        ...
        gen_loss.backward()
        nn.utils.clip_grad_norm_(self.net_h2c.parameters(), max_norm=20, norm_type=2)
        nn.utils.clip_grad_norm_(self.net_c2h.parameters(), max_norm=20, norm_type=2)
        self.optimizer.step()
