...
from .blocks import  Interpolate, _make_scratch, _make_pretrained_efficientnet_lite3, FeatureFusionBlock_custom
import os
...
class HazeRemovalNet(BaseNetwork):
    ...
    def forward_get_A(self, x): # output A: N,3,1,1
        if self.use_dc_A:
            A = self.dcgf_tool.get_atmosphere_light_new(x)
        else:
...
class HazeProduceNet(BaseNetwork):
    ...
    def forward_random_parameters(self, x, d):
        n,c,h,w = x.shape
